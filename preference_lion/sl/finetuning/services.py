...
async def _run_openai_finetuning_job(
    ...
    while True:
        ...
        if job_status.status == "succeeded":
            ...
            # Update oai_job with the latest job status to get fine_tuned_model
            oai_job = job_status
            break
    ...
    if oai_job.fine_tuned_model is None:
        logger.error(f"Finetuning job {oai_job.id} succeeded but fine_tuned_model is None. Job details: {oai_job}")
        raise RuntimeError(f"Finetuning job succeeded but fine_tuned_model is None")
